...
public class BattlePage extends JFrame {
    ...
    private final LinkedList<String> battleHistory;
    ...
    public BattlePage(HomePage homePage, PvECampaign campaign, BattleMode mode, int partyLevel) {
        ...
        recoverButton = new JButton("Recover 5 Health");
    ...
    private void doAction(Command command) {
        ...
        engine.performPlayerAction(new AttackCommand(command, targetIndex));
        handlePostAction();
    }
    ...
    private void processAutoTurns() {
        engine.performAutomaticTurnsUntilPlayerNeeded();
    }
    ...
    private void refreshUI() {
        ...
        for (Unit unit : getTargetParty().getUnits()) {
            targetBox.addItem(unit.getName());
        }
    ...
    private Party getTargetParty() {
        return engine.isCurrentTurnLeftSide() ? engine.getEnemiesParty() : engine.getHeroesParty();
    }
