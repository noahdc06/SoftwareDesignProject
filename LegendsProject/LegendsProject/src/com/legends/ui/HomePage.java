...
public class HomePage extends JFrame {
    ...
    public void refreshInfo() {
        if (sessionManager.isLoggedIn()) {
            ...
            infoLabel.setText("Logged in as " + profile.getUsername() + " | Level " + profile.getPartyLevel() + " | Room " + profile.getRoomCount());
        } else {
    ...
    private void startPvE() {
        if (!sessionManager.isLoggedIn()) {
            JOptionPane.showMessageDialog(this, "Login first to play PvE.");
            return;
        }
        Profile profile = sessionManager.getCurrentProfile();
        PvECampaign campaign = new PvECampaign(profile);
        BattlePage.startPvEFlow(this, campaign);
    }
}
