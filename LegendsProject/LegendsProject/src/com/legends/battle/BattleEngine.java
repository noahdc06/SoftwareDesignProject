...
public class BattleEngine {
    ...
    public BattleEngine(int partyLevel, BattleMode mode) {
        ...
        createParties(partyLevel);   // build hero + enemy teams
        buildTurnOrder();            // define turn sequence

        this.turnIndex = 0;
        advanceToNextActiveTurn();   // skip downed units at start
    }
    ...
    // Rename heroes for PVP clarity (e.g., "Rogue 1", "Rogue 2")
    ...
    // Fixed alternating turn order (hero → enemy → hero → enemy...)
    ...
    // Return action labels dynamically based on hero
    ...
    // Delegates description to Strategy pattern
    ...
    // Handles player input using Command pattern
    public void performPlayerAction(AttackCommand action) {
        if (finished || !isCurrentTurnPlayerControlled()) return;
    ...
    // Derives party level from hero stats
    ...
    // Simple enemy AI: always attack random target
    ...
    // Determines if battle is finished
    ...
    // Restore all heroes (used in Inn)
    ...
    // Strategy selection based on hero class
